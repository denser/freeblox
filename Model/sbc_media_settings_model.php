...
*/
	...
	class SBC_Media_SettingsModel {
		...
		public static $MEDIA_PORTS = 'MEDIA_PORTS';

		public function get($name) {
			$dbh = DB_Handle::Connect();
			if ( !isset(self::$get_stmt) ) {
				self::$get_stmt = $dbh->prepare('SELECT VAL FROM MEDIA_PORT_SETTINGS 
							WHERE NAME = :name;');
			}
			self::$get_stmt->reset();
		...
		public function set($name, $val) {
			...
			if ( !isset(self::$set_stmt) ) {
				self::$set_stmt = $dbh->prepare('UPDATE MEDIA_PORT_SETTINGS SET 
