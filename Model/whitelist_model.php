...
*/
	...
	class WhiteListConfigModel {
		...
		public function getWhiteListRules() {
			...
			if ( !isset(self::$get_stmt) ) {
				self::$get_stmt = $dbh->prepare('SELECT NAME, IP_TYPE, NET_ADDRESS, 
							ENABLED, COMMENTS FROM WHITELIST_RULES');
			}
			self::$get_stmt->reset();
		...
		public function getWhitelistRule($name) {
			$dbh = DB_Handle::Connect();
			if ( !isset(self::$get_stmt) ) {
				self::$get_stmt = $dbh->prepare('SELECT NAME, IP_TYPE, NET_ADDRESS, 
							ENABLED, COMMENTS FROM WHITELIST_RULES WHERE 
							NAME = :name;');
			}
			self::$get_stmt->reset();
		...
		public function checkIfRuleExists($name) {
			$dbh = DB_Handle::Connect();
			return $dbh->querySingle("SELECT count(*) FROM WHITELIST_RULES 
						WHERE NAME = '$name'");
		}

		public function addWhiteListRule($name, $ip_type, $netaddr, $enabled, $comments) {
			$dbh = DB_Handle::Connect();
			if ( self::checkIfRuleExists($name) != 0 ) {
				return FALSE;
			}
			if ( !isset(self::$add_stmt) ) {
				self::$add_stmt = $dbh->prepare('INSERT INTO WHITELIST_RULES (NAME, 
							IP_TYPE, NET_ADDRESS, ENABLED, COMMENTS) 
							VALUES (:name, :ip_type, :netaddr, :enabled, 
							:comments)');
			}
			self::$add_stmt->reset();
		...
		public function editWhiteListRule($name, $ip_type, $netaddr, $enabled, $comments) {
			$dbh = DB_Handle::Connect();
			if ( self::checkIfRuleExists($name) != 1 ) {
				return FALSE;
			}
			if ( !isset(self::$edit_stmt) ) {
				self::$edit_stmt = $dbh->prepare('UPDATE WHITELIST_RULES SET 
		...
		public function setWhiteListEnable( $name, $enabled ) {
			$dbh = DB_Handle::Connect();
			if ( self::checkIfRuleExists($name) != 1 ) {
				return FALSE;
			}
		...
		public function deleteWhiteListRule($name) {
			$dbh = DB_Handle::Connect();
			if ( !isset(self::$del_stmt) ) {
				self::$del_stmt = $dbh->prepare('DELETE FROM WHITELIST_RULES 
							WHERE NAME = :name;');
			}
			self::$del_stmt->reset();
		...
		public function getNames() {
			$dbh = DB_Handle::Connect();
			$res = $dbh->query("SELECT NAME FROM WHITELIST_RULES");
			$i=0;
