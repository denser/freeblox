...
*/
	...
	class RadiusSettingsModel {
		...
		public static $ENABLE = 'EnableRadius';
		public static $SERVER_IP = 'ServerIP';
		public static $SERVER_PORT = 'ServerPort';

		public function get($name) {
			$dbh = DB_Handle::Connect();
			if ( !isset(self::$get_stmt) ) {
				self::$get_stmt = $dbh->prepare('SELECT VAL FROM RADIUS 
							WHERE NAME = :name;');
			}
			self::$get_stmt->reset();
		...
		public function set($name, $val) {
			...
			if ( !isset(self::$set_stmt) ) {
				self::$set_stmt = $dbh->prepare('UPDATE RADIUS SET VAL = :val 
							WHERE NAME = :name;');
			}
			self::$set_stmt->reset();
