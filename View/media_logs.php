...
*/
	...
	$MEDIA_LOGS_FILE="/var/log/rtpengine.log";
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript" charset="utf-8">
			...
			function edit_data(val,id) {
				parent.callEditPage(val, 'edit_email_settings.php?id=media_logs', 360, 650);
			}
		</script>
		<?php
			if ( isset($_POST['media_logs_interval']) ) {
				$page_refresh_interval = $_POST['media_logs_interval'];
			}
			else if ( isset($_GET['media_logs_interval']) ) {
				$page_refresh_interval = $_GET['media_logs_interval'];
			} else {
		...
		<script type="text/javascript">
			function status_refresh() {
				location.replace("media_logs.php?media_logs_interval=" + refresh_interval_frm_srv);
			}

			function submitform() {
				if($sub("#MediaLogs").validationEngine('validate')) {
			...
			function initValidation() {
				$sub( document ).ready(function() {
					$sub("#media_logs_interval").attr("data-validation-engine","validate[required,custom[integer],min[15],max[3600]]");
					$sub("#MediaLogs").validationEngine('attach', {promptPosition : "bottomRight"});
				});
			}
	...
	<body onclick="javascript: parent.hideMenu()">
		<div class="content" style="padding-right:20px;">
			<h1>Media Logs<img src="images/icn_help.png" style="margin-left: 10px; cursor: pointer" onclick="parent.helpPage()" /></h1>
			<form name="MediaLogs" id="MediaLogs" method="POST" action="media_logs.php" autocomplete="off">
								...
								<tr>
									<td>
										<table cellpadding="2" cellspacing="2"  class="sub-container-table">
											<tr>
												<td><input type="text" maxlength=4; size=4; class="ipBox" id="media_logs_interval" name="media_logs_interval" value="<?php echo $page_refresh_interval; ?>" /></td>
				...
				<table id="status_alerts" class="tablesorter" cellpadding="0" cellspacing="1px">
					...
					<tbody><?php $logs = SysCalls::read_logs($MEDIA_LOGS_FILE,1000,0);
						$count =count($logs);
						for($j=0;$j < $count;$j++) {
					?>
