...
*/
	...
	if(isset($_POST) && isset($_POST['submit_action'])) {
		...
		if($_POST['submit_action'] == "3") {
			if( $dependancy == 0) {
				$obj1 -> delRules($_POST["TRUNK_CONFIG"]);
				$obj1->createActionList(DEPLOY_MODULES::LBRULE_PROFILE_CONFIG, 
						$_POST["TRUNK_CONFIG"]." Rule deleted", 
						DEPLOY_MODULES::LBRULE_PROFILE_CONFIG_STR, "");
				$res =1;
			} else {
			...
			for($i = 0; $i < count($del_arr); $i++) {
				if( $dependancy == 0) {
					$obj1 -> delRules($del_arr[$i]);
					$obj1->createActionList(DEPLOY_MODULES::LBRULE_PROFILE_CONFIG, $del_arr[$i]." Rules deleted", DEPLOY_MODULES::LBRULE_PROFILE_CONFIG_STR, "");
				} else {
					if($depname == "") {
						$depname .= $del_arr[$i];
					} else {
	...
	if(isset($_POST) && isset($_POST['submit_action'])) {
		if ($_POST['submit_action'] == "2") {
			...
			$data_array = array(
				"NAME" => $_POST['NAME'],
				"RESOURCE_NAMES" => $post_trunks,
				"LB_GRP_NAME" => $_POST['LB_GRP_NAME']
				);
			$obj1 -> addrules($data_array);
			if ($_POST['CON_1_LIST']!="") {
		...
		if ($_POST['submit_action'] == "1") {
			...
			$obj1 -> setrules($_POST['NAME'], $data_array);
			$obj1 -> deletecondition($data_array);
			if($_POST['CON_1_LIST']!=""){
				$con_1_str = explode(",", $_POST['CON_1_LIST']);
				$con_11_hd_str = explode(",", $_POST['CON_11_HD_LIST']);
				$con_11_pattern_str = explode(",", $_POST['CON_11_PATTERN_LIST']);
	...
	<head>
		...
		<script type="text/javascript">
			...
			function edit_data(val) {
				parent.callEditPage(val, 'add_edit_lbrule_profile.php',360, 800);
			}

			function add_data() {
				parent.callAddPage('add_edit_lbrule_profile.php', 360, 800);
			}
			...
			function refreshIframe(val) {
				if(val == 1) {
					parent.document.getElementById("container").src = "lbrule_profile.php";
				}
			}
		...
		<div class="content">
			<h1>LB Rules Settings<img src="images/icn_help.png" style="margin-left: 10px; cursor: pointer" onclick="parent.helpPage()" /></h1>
					...
					<table id="manipulationtablerules" class="tablesorter" cellpadding="0" cellspacing="1px">
						<thead>
							<tr>
								...
								<th><a href="#">Resource Names</a></th>
								<th><a href="#">Group Names</a></th>
						...
						while($row = $result->fetchArray(SQLITE3_ASSOC)) {
							$i++;
							$rules_data = $obj1 -> getconditions($row["NAME"]);
						...
						<tr>
							...
							<td><?php echo $row['RESOURCE_NAMES'] ?></td>
