...
*/
	...
	$SIGNAL_LOGS_FILE="/var/log/opensips.log";
	...
	<head>
		...
		<script type="text/javascript" charset="utf-8">
			...
			function edit_data(val) {
				parent.callEditPage(val, 'edit_email_settings.php?id=signal_logs', 360, 650);
			}
		</script>
<!-- Sort function -->
		<?php
			if ( isset($_POST['signal_interval']) ) {
				$page_refresh_interval = $_POST['signal_interval'];
			} else if ( isset($_GET['signal_interval']) ) {
				$page_refresh_interval = $_GET['signal_interval'];
			} else {
		...
		<script type="text/javascript">
			function status_refresh() {
				location.replace("signal_logs.php?signal_interval=" + refresh_interval_frm_srv);
			}

			function submitform() {
				if($sub("#SignalLogs").validationEngine('validate')) {
			...
			function initValidation() {
				$sub( document ).ready(function() {
					$sub("#signal_interval").attr("data-validation-engine","validate[required,custom[integer],min[15],max[3600]]");
					$sub("#SignalLogs").validationEngine('attach', {promptPosition : "bottomRight"});
				});
			}
	...
	<body onclick="javascript: parent.hideMenu()">
		<div class="content" style="padding-right:20px;">
			<h1>Signalling Logs<img src="images/icn_help.png" style="margin-left: 10px; cursor: pointer" onclick="parent.helpPage()" /></h1>
			<form name="SignalLogs" id="SignalLogs" method="POST" action="signal_logs.php" autocomplete="off">
									...
									<td>
										<table cellpadding="2" cellspacing="2"  class="sub-container-table">
											<tr>
												...
												<td><input type="text" maxlength=4; size=4; class="ipBox" id="signal_interval" name="signal_interval" value="<?php echo $page_refresh_interval; ?>" /></td>
												...
												<td>
													<a class="btn-grey-wide-l" id="update" onclick="edit_data('<?php echo "signal_logs"; ?>');" ><span>E-mail Server Settings</span></a>
												</td>-->
											</tr>
											</form>
										</table>
...
<!-- Begin: Status Alerts logs Contnet -->
				<table id="status_alerts" class="tablesorter" cellpadding="0" cellspacing="1px">
					<thead>
					<tr>
						<th scope="col" width="15%"><a href="#">Date</a></th>
						<th scope="col" width="85%"><a href="#">Log Msg</a></th>
					</tr>
					</thead>
					<tbody><?php $logs = SysCalls::read_logs($SIGNAL_LOGS_FILE,1000,0);
						$count =count($logs);
						for($j=0;$j < $count;$j++) {
						?>
