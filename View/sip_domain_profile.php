...
*/
	...
	$_SESSION['pageID'] = "#sip_domain_settings";
	$obj = DateTimeConfig::getConfig();
	$update_count = $obj->getActionListCount();
	$subscribers_settings_config = SipDomainSettingsModel::getInstance();
	$subscribers_settings = $subscribers_settings_config->getAllsipdomainSettings();
	...
	if(isset($_POST) && isset($_POST['submit_action'])) {
		...
		if($_POST['submit_action'] == "3") {
			$delcheck = $subscribers_settings_config->checksipprofile($_POST["DELETE_RULE"]);
			if($delcheck == 0) {
				$subscribers_settings_config->deletesipdomainSettings($_POST["DELETE_RULE"]);
				$subscribers_settings_config->createActionList(DEPLOY_MODULES::SIPDOMAIN_PROFILE_CONFIG, "SIP Domain Profile deleted", DEPLOY_MODULES::SIPDOMAIN_PROFILE_CONFIG_STR, "");
					$delete = 1;
			} else {
		...
		if($_POST['submit_action'] == "4") {
			...
			for($i = 0; $i < count($del_arr); $i++) {
				$delcheck = 0;
				$delcheck = $subscribers_settings_config->checksipprofile($del_arr[$i]);
				if($delcheck == 0) {
				$set = 1;
				$subscribers_settings_config->deletesipdomainSettings($del_arr[$i]);
				} else {
			...
			if($set != 0) {
				$subscribers_settings_config->createActionList(
					...
					" All Sip domain Profiles deleted", 
		...
		if ($dset == 1) {
			...
			print "Can't delete the $str Profiles as they are being used by SIP profiles .";
			print "</Div>";
			$delete = 0;
		}
	}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			function edit_data(val) {
				parent.callEditPage(val, 'add_edit_sip_domain_profile.php', 360, 600);
			}

			function add_data() {
				parent.callAddPage('add_edit_sip_domain_profile.php', 360, 600);
			}
	...
	<body onclick="javascript: parent.hideMenu();" onload="refreshIframe('<?php print $delete; ?>');">
		<div class="content">
			<h1>SIP Domain Profile Settings<img src="images/icn_help.png" style="margin-left: 10px; cursor: pointer" onclick="parent.helpPage()" /></h1>
			<div id="roaming_config_body" style="height: ; overflow-y: hidden; width: 95%; overflow-x: hidden">
				<table border="0" cellpadding="0" cellspacing="0" style="margin: 0px" class="table-config no-border">
					<table id="resourcesettings" class="tablesorter" cellpadding="0" cellspacing="1px">
						<thead>
							<tr>
								<td width="25px"><input type="checkbox" id="checkall" align="center" /></td>
								<th><a href="#">SIP Domain</a></th>
						...
						?>
							<tr>
								<td><input id="checkbox<?php echo $i ?>" name="chkbox" type='checkbox' align="center" value="<?php echo $row[SipDomainSettingsModel::$ID]; ?>" onclick="uncheck()" /></td>
								<td><?php echo $row[SipDomainSettingsModel::$SIP_DOMAIN]; ?></td>
								<td align="left">
									<input type="button" class="btn-edit edit-deviceaccesslist" onclick="edit_data('<?php echo $row[SipDomainSettingsModel::$ID]; ?>')" onfocus="if(this.blur)this.blur()" />
									<input type="button" class="btn-delete" onclick="delete_data('<?php echo $row[SipDomainSettingsModel::$ID]; ?>')" onfocus="if(this.blur)this.blur()" />
								</td>
							</tr>
						<?php
								}
