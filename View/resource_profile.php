...
*/
	...
	$subscribers_settings_config = ResourceSettingsModel::getInstance();
	...
	if(isset($_POST) && isset($_POST['submit_action'])) {
		...
		if($_POST['submit_action'] == "3") {
			if($delcheck == 0) {
				...
				$subscribers_settings_config->createActionList(DEPLOY_MODULES::RESOURCE_PROFILE_CONFIG, "Resource Profile deleted", DEPLOY_MODULES::RESOURCE_PROFILE_CONFIG_STR, "");
					$delete = 1;
			} else {
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			function edit_data(val) {
				parent.callEditPage(val, 'add_edit_resource_profile.php', 360, 600);
			}

			function add_data() {
				parent.callAddPage('add_edit_resource_profile.php', 360, 600);
			}
	...
	<body onclick="javascript: parent.hideMenu();" onload="refreshIframe('<?php print $delete; ?>');">
		<div class="content">
			<h1>Resource Profile Settings<img src="images/icn_help.png" style="margin-left: 10px; cursor: pointer" onclick="parent.helpPage()" /></h1>
			<div id="roaming_config_body" style="height: ; overflow-y: hidden; width: 95%; overflow-x: hidden">
				<table border="0" cellpadding="0" cellspacing="0" style="margin: 0px" class="table-config no-border">
					<table id="resourcesettings" class="tablesorter" cellpadding="0" cellspacing="1px">
						...
						?>
							<tr>
								<td><input id="checkbox<?php echo $i ?>" name="chkbox" type='checkbox' align="center" value="<?php echo $row[ResourceSettingsModel::$ID]; ?>" onclick="uncheck()" /></td>
								<td><?php echo $row[ResourceSettingsModel::$RESOURCE_NAME]; ?></td>							
								<td align="left">
									<input type="button" class="btn-edit edit-deviceaccesslist" onclick="edit_data('<?php echo $row[ResourceSettingsModel::$ID]; ?>')" onfocus="if(this.blur)this.blur()" />
									<input type="button" class="btn-delete" onclick="delete_data('<?php echo $row[ResourceSettingsModel::$ID]; ?>')" onfocus="if(this.blur)this.blur()" />
								</td>
							</tr>
							<?php
								}
