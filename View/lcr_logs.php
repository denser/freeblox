...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript" charset="utf-8">
			...
			function edit_data(val) {
				parent.callEditPage(val, 'edit_email_settings.php?id=lcr_logs', 360, 650);
			}
		</script>
		<?php
			if ( isset($_POST['lcr_logs_interval']) ) {
				$page_refresh_interval = $_POST['lcr_logs_interval'];
			}
			else if ( isset($_GET['lcr_logs_interval']) ) {
				$page_refresh_interval = $_GET['lcr_logs_interval'];
			} else {
		...
		<script type="text/javascript">
			function status_refresh() {
				location.replace("lcr_logs.php?lcr_logs_interval=" + refresh_interval_frm_srv);
			}

			function submitform() {
				if($sub("#LcrLogs").validationEngine('validate')) {
			...
			function initValidation() {
				$sub( document ).ready(function() {
					$sub("#lcr_logs_interval").attr("data-validation-engine","validate[required,custom[integer],min[15],max[3600]]");
					$sub("#LcrLogs").validationEngine('attach', {promptPosition : "bottomRight"});
				});
			}
	...
	<body onclick="javascript: parent.hideMenu()">
		<div class="content" style="padding-right:20px;">
			<h1>LCR Logs<img src="images/icn_help.png" style="margin-left: 10px; cursor: pointer" onclick="parent.helpPage()" /></h1>
			<form name="LcrLogs" id="LcrLogs" method="POST" action="lcr_logs.php" autocomplete="off">
<!-- Begin: Log Viewer Settings Content -->
									...
									<td>
										<table cellpadding="2" cellspacing="2"  class="sub-container-table">
											<tr>
												...
												<td><input type="text" maxlength=4; size=4; class="ipBox" id="lcr_logs_interval" name="lcr_logs_interval" value="<?php echo $page_refresh_interval; ?>" /></td>
				...
				<table id="status_alerts" class="tablesorter" cellpadding="0" cellspacing="1px">
					...
					<tbody><?php $logs = SysCalls::read_logs($LCR_LOG_FILE,1000,0);
						$count =count($logs);
						for($j=0;$j < $count;$j++) {
					?>
